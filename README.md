...
- **Voice Orchestration Engine** — Built the Vapi-powered outbound call orchestration system with cloned voice support, real-time transcript streaming, and automatic call event persistence via Convex mutations.
- **Convex Backend Architecture** — Designed and implemented the serverless Convex backend with real-time document subscriptions, issue tracking, call event correlation, and webhook ingestion for Vapi call lifecycle events.
- **Founder Oversight Dashboard** — Developed the Next.js dashboard with live call monitoring, transcript streaming, issue management, and one-click call initiation with configurable voice personas.
- **Agent Loop Design** — Architected the agentic call flow including context injection, dynamic prompt assembly from issue data, and post-call summary extraction.
